...
| `06` Art & UX | A **modern "strategy console" UI** with a **light/dark theme toggle** (🌙/☀) — stat chips, clean system-sans type, a cinnabar accent with jade/amber semantics, layered shadows and glassy theme-aware overlays. The map is a **stylized China landmass** (coastline over sea) with real **depth**: soft **faction territory shading** that builds regions, **glossy city tokens** with drop shadows and highlights, a **layered river**, fog-of-war markers, and a vignette. Plus click-the-map UX, alert log, modal events, battle HUD, scenario/faction select, and tutorial |
