...
| `04` Intrigue & recon | A strategic **espionage** system (🗡 from any bordering city): **sabotage** a garrison, **incite unrest**, **subvert** an enemy officer's loyalty, **turn** him to your side, or **assassinate** him — success pits your spymaster's Intellect against enemy counter-intel. Plus a **Scout** action (🔍) to reveal any fogged region of the map for a few turns |
...
| `03` Personnel | **Move officers** between your cities (once per turn, no AP), and a searchable **Officer Roster** (👥) to find any officer by name — respecting fog (your officers plus any in territory you can see) |
| Fog of war | Enemy details are **hidden until you can see them** — cities beyond your borders show as **"?"**; adjacent/owned cities reveal live garrison and officers, and discovered-but-distant cities are remembered. Expansion and intrigue reveal the map. |
...
| `06` Art & UX | A **modern "strategy console" UI** with a **light/dark theme toggle** (🌙/☀) — stat chips, clean system-sans type, a cinnabar accent with jade/amber semantics, layered shadows and glassy theme-aware overlays. The map is a **political map of China**: a realistic coastline over sea, tiled into **province shapes** (Voronoi regions) filled by their owner's color, with **frontier borders** bolded between rival territories, thin internal province lines, a layered river, fog-of-war markers, glossy city tokens, and a vignette. Plus click-the-map UX, alert log, modal events, battle HUD, scenario/faction select, and tutorial |
